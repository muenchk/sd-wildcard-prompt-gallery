...
class Toprow:
    ...
    selection_wildcard = None
    selection_refresh_wildcards = None
    prompt = None
    prompt_wildcard = None
    prompt_second = None
    prompt_combined = None
    prompt_img = None
    ...
    def __init__(self, is_img2img, _wildcards:list[str], is_compact=False, id_part=None):
        if id_part is None:
    ...
    def create_prompts(self):
        with gr.Column(elem_id=f"{self.id_part}_prompt_container", elem_classes=["prompt-container-compact"] if self.is_compact else [], scale=6):
            with gr.Row():
                self.selection_wildcard = gr.Dropdown(value="No", choices=self.wildcards, label="Select wildcard", elem_id=f"{self.id_part}_wildcard_dropdown")
                self.selection_refresh_wildcards = ToolButton(value=refresh_symbol, elem_id=f"{self.id_part}_refresh_wildcards")
            with gr.Row(elem_id=f"{self.id_part}_prompt_row", elem_classes=["prompt-row"]):
                self.prompt = gr.Textbox(label="Beginning of prompt", elem_id=f"{self.id_part}_prompt", show_label=True, lines=3, placeholder="Prompt\n(Press Ctrl+Enter to generate, Alt+Enter to skip, Esc to interrupt)", elem_classes=["prompt"], value='')
                self.prompt_img = gr.File(label="", elem_id=f"{self.id_part}_prompt_image", file_count="single", type="binary", visible=False)
            with gr.Row(elem_id=f"{self.id_part}_prompt_wildcard_row", elem_classes=["prompt-wildcard-row"]):
                self.prompt_wildcard = gr.Textbox(label="Wildcard", elem_id=f"{self.id_part}_prompt_second", show_label=True, lines=1, interactive=False, elem_classes=["prompt"], value='')
            with gr.Row(elem_id=f"{self.id_part}_prompt_second_row", elem_classes=["prompt-second-row"]):
                self.prompt_second = gr.Textbox(label="End of prompt", elem_id=f"{self.id_part}_prompt_second", show_label=True, lines=3, placeholder="Prompt\n(Press Ctrl+Enter to generate, Alt+Enter to skip, Esc to interrupt)", elem_classes=["prompt"], value='')
                self.prompt_combined = gr.Textbox(visible=False,value="")

            with gr.Row(elem_id=f"{self.id_part}_neg_prompt_row", elem_classes=["prompt-row"]):
                self.negative_prompt = gr.Textbox(label="Negative prompt", elem_id=f"{self.id_part}_neg_prompt", show_label=True, lines=3, placeholder="Negative prompt\n(Press Ctrl+Enter to generate, Alt+Enter to skip, Esc to interrupt)", elem_classes=["prompt"], value='')
    ...
    def create_submit_box(self):
        ...
        with gr.Row(elem_id=f"{self.id_part}_generate_alt_box", elem_classes=["generate-box"] + (["generate-box-compact"] if self.is_compact else []), render=not self.is_compact) as submit_alt_box:
            self.submit_alt_box = submit_alt_box

            self.submit_samples = gr.Button('Generate Samples', elem_id=f"{self.id_part}_generate_samples", variant='secondary')
            self.submit_samples_save = gr.Button('Generate Samples and Save', elem_id=f"{self.id_part}_generate_samples_save", variant='secondary')
